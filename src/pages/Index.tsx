...
const Index = () => {
  ...
  const { entries, currentRate, isLoading, editingEntry, addEntry, editEntry, cancelEdit, deleteEntry } = useBitcoinEntries();
              ...
              {isLoading ? (
                ...
                entries.map((entry) => (
                  <TableRow key={entry.id}>
                    <TableCell className="font-medium text-right">{format(entry.date, 'dd/MM/yyyy', { locale: ptBR })}</TableCell>
                    <TableCell className="text-right">
                      {entry.currency === 'USD' ? '$' : 'R$'} {formatNumber(entry.amountInvested)}
                    </TableCell>
                    <TableCell className="text-right hidden sm:table-cell">{formatNumber(entry.btcAmount, 8)} BTC</TableCell>
