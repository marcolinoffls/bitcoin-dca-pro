...
export const fetchBitcoinPriceHistory = async (range: '1D' | '7D' | '1M' | '1Y' | 'ALL') => {
  try {
    // Mapeia o número de dias conforme o range escolhido
    const daysMap = {
      '1D': 1,
      '7D': 7,
      '1M': 30,
    ...
    // Define o intervalo ideal para o gráfico:
    // - 1D → minutely (se possível)
    // - 7D → hourly
    // - 1M → hourly
    // - 1Y, ALL → daily
    let interval = 'hourly';
    ...
    // Faz requisição na API da CoinGecko
    const response = await fetch(
    ...
    // Formata os dados para o formato usado no gráfico
    ...
    // Tentativa de fallback via CoinMarketCap
    try {
      const coinMarketCapApiKey = import.meta.env.VITE_CMC_API_KEY;
      ...
      // Simula dados simples baseados no preço atual
      return [{
        time: now.toLocaleDateString('pt-BR'),
        price: parseFloat(currentPrice.toFixed(2))
      }];
    } catch (fallbackError) {
      console.error("Erro também no fallback da CoinMarketCap:", fallbackError);

      // Se tudo falhar, devolve um dado mínimo para não quebrar o gráfico
