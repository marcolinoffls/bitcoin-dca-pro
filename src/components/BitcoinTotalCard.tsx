...
const BitcoinTotalCard: React.FC<BitcoinTotalCardProps> = ({
  ...
  const formattedTotalBitcoin = displayUnit === 'SATS' ? formatNumber(totalBitcoin * 100000000, 0) : formatNumber(totalBitcoin, 8);

  // Usa verificação de segurança ao acessar currentRate
  const currentRateValue = isCurrencyDataAvailable ? selectedCurrency === 'USD' ? currentRate.usd : currentRate.brl : 0;
  ...
  const totalInvested = entries.reduce((total, entry) => {
    ...
    if (entry.currency !== selectedCurrency && isCurrencyDataAvailable) {
      ...
      return total + (entry.currency === 'USD' ? entryValue * (1 / conversionRate) : entryValue * conversionRate);
    }
    return total + entryValue;
  }, 0);
  const percentChange = totalInvested > 0 ? (totalValueCurrent - totalInvested) / totalInvested * 100 : 0;
  const isPositiveChange = percentChange >= 0;

  // Classes para transição suave
  const hiddenClass = isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-2 absolute';
  const dotsClass = isVisible ? 'opacity-0 -translate-y-2 absolute' : 'opacity-100 translate-y-0 relative';
  return <Card className="overflow-hidden rounded-2xl shadow-md hover:shadow-lg transition-shadow duration-800 h-[220px]"> {/* Adicionado h-[250px] */}
      <CardContent className="p-5">
        <div className="flex items-start justify-between mb-4">
          <div className="flex items-center gap-3">
            <div className="h-8 w-8 flex-shrink-0">
              <img src="https://wccbdayxpucptynpxhew.supabase.co/storage/v1/object/sign/icones/bitcoin.png?token=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCIsImtpZCI6InN0b3JhZ2UtdXJsLXNpZ25pbmcta2V5XzkxZmU5MzU4LWZjOTAtNDJhYi1hOWRlLTUwZmY4ZDJiNDYyNSJ9.eyJ1cmwiOiJpY29uZXMvYml0Y29pbi5wbmciLCJpYXQiOjE3NDQ0OTI3ODgsImV4cCI6MTc3NjAyODc4OH0.lrqzizN8_y_ZR8PExIB_XH6xBoqeG0OvxlwMbv0TtF0" alt="Bitcoin" className="h-full w-full object-contain" />
            </div>
            <div>
          ...
          <div className="flex items-center gap-2">
            {entries.length > 0 && <Badge className={`${isPositiveChange ? 'bg-green-100 text-green-600 hover:bg-green-100' : 'bg-red-100 text-red-600 hover:bg-red-100'} flex items-center gap-0.5 px-2 py-1 border-0`}>
                {isPositiveChange ? <ArrowUp className="h-3 w-3" /> : <ArrowDown className="h-3 w-3" />}
                {Math.abs(percentChange).toFixed(1)}%
              </Badge>}
            
            {/* Botão de alternar visibilidade do saldo */}
            <button onClick={toggleVisibility} className="text-gray-400 hover:text-gray-600 transition-colors duration-200" title={isVisible ? "Ocultar saldo" : "Mostrar saldo"} aria-label={isVisible ? "Ocultar saldo" : "Mostrar saldo"}>
        ...
        <div className="flex justify-between items-end mt-4">
          <div className="flex flex-col">
            <p className="text-gray-400 mb-1 text-sm font-bold">Portfólio</p>
                <div className="relative">
                  <p className={`text-2xl font-bold transition-all duration-300 ease-in-out flex gap-1 ${hiddenClass}`}>
                    <span className="text-sm align-top mt-0.5">{currencySymbol}</span>
            ...
            <div className="relative mt-1">
              <div className={`flex flex-col transition-all duration-300 ease-in-out ${hiddenClass}`}>
                <span className="text-sm text-gray-400 font-bold">Total investido:</span>
          ...
          <div className="relative">
            <p className={`text-base font-semibold transition-all duration-300 ease-in-out ${hiddenClass}`}>
              {formattedTotalBitcoin} {displayUnit}
            </p>
            <p className={`text-base font-semibold transition-all duration-300 ease-in-out ${dotsClass}`}>
              ●●●●
            </p>
          </div>
        </div>
